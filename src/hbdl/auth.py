...
def _save_session(cookie_value: str, path: Path = config.SESSION_FILE) -> None:
    config.ensure_dirs()
    path.write_text(json.dumps({"cookie": cookie_value}), encoding="utf-8")
    os.chmod(path, 0o600)


def _load_saved_session(path: Path = config.SESSION_FILE) -> Session | None:
    if not path.exists():
        return None
...
def resolve_session(
    ...
    if cookie_file:
        ...
        raise AuthError(f"Kein '{SESSION_COOKIE_NAME}'-Cookie in {cookie_file} gefunden.")
...
def check_session(session: Session) -> dict:
    ...
    if resp.status_code in (401, 403):
        raise AuthError("Cookie ungueltig oder abgelaufen -- `hbdl auth login` erneut ausfuehren.")
